        ...
        { !value || value.count() < 1 ? null :
          ...
          return (
            <div key={i} className="json-schema-form-item">
              ...
              <Button type="button" className="btn btn-sm json-schema-form-item-remove" onClick={()=> this.removeItem(i)} > - </Button>
            </div>
            )
          }).toArray()
        }
        <Button type="button" className={`btn btn-sm json-schema-form-item-add ${errors.length ? "invalid" : null}`} onClick={this.addItem}> Add item </Button>
      </div>
    )
  }
}
